...
std::unordered_map<int,std::multimap<double, IntersectionIndex>> closest_intersections_map(std::vector<int> startIntersections,std::vector<int> endIntersections){
    double length;
    ...
    for (int i = 0; i < startIntersections.size(); i++){
        ...
        for(int j = 0; j < endIntersections.size(); j++){
            ...
            int currEnd = endIntersections[j];
            std::pair<LatLon,LatLon> points;
            LatLon start =  getIntersectionPosition(currIntersection);
