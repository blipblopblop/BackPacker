...
void reverseIntVector(std::vector<doubke>& vector)
{
                            ...
                            const double walking_time_limit)
{
    ...
    intersectionsToVisit.insert( std::make_pair(1, startingNode) );
    ...
    while (!intersectionsToVisit.empty())
    {
        ...
        for (auto it = connectedStreetSegments.begin(); it != connectedStreetSegments.end(); it++)
        {
            ...
            travelTime = travelTime - compute_path_walking_time(newSegment, walking_speed, turn_penalty);
